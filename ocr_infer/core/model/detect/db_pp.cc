...
void DbPostprocessing::Parse(const cv::Mat &pred_map, const cv::Point2f &scales,
  ...
  for (int i = 0; i < num_contours; i++) {
    ...
    if (std::min(box1.size.height, box1.size.width) < this->min_side) continue;
    float mean_score = this->GetMeanScore(pred_map, box1);
    if (mean_score < this->box_thresh) continue;
    ...
    cv::Point2f p[4];
    box2.points(p);
    if (std::min(box2.size.height, box2.size.width) < this->min_side + 2) continue;
    out->emplace_back(box2);
  }
}

// TODO: 优化加速
cv::Mat DbPostprocessing::PartialVectorToMat(const cv::Mat &v, int y_start, int y_end, int x_start,
                                             int x_end) {
  cv::Mat mat(y_end - y_start, x_end - x_start, CV_32FC1);
  for (int i = 0; i < mat.rows; i++)
    for (int j = 0; j < mat.cols; j++) mat.at<float>(i, j) = v.at<float>(y_start + i, x_start + j);
  return mat;
}

float DbPostprocessing::GetMeanScore(const cv::Mat &pred_map, const cv::RotatedRect &box) {
  int height = pred_map.rows, width = pred_map.cols;
  cv::Point2f points[4];
  box.points(points);
  float f_xmin = std::min_element(points, points + 3, [](cv::Point2f p1, cv::Point2f p2) {
                   return p1.x < p2.x;
                 })->x;
  float f_xmax = std::max_element(points, points + 3, [](cv::Point2f p1, cv::Point2f p2) {
                   return p1.x < p2.x;
                 })->x;
  float f_ymin = std::min_element(points, points + 3, [](cv::Point2f p1, cv::Point2f p2) {
                   return p1.y < p2.y;
                 })->y;
  float f_ymax = std::max_element(points, points + 3, [](cv::Point2f p1, cv::Point2f p2) {
                   return p1.y < p2.y;
                 })->y;
  ...
  cv::Mat pred_mat = this->PartialVectorToMat(pred_map, ymin, ymax + 1, xmin, xmax + 1);
  cv::Scalar s = cv::mean(pred_mat, mask);
  return static_cast<float>(s[0]);
}

std::vector<cv::Point> DbPostprocessing::Unclip(const cv::RotatedRect &box, float Unclip_ratio) {
...
inline void DbPostprocessing::MapToOriginImage(const cv::Point2f &scales,
