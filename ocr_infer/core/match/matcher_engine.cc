...
int MatcherEngine::AddKeyword(KeywordId id, const std::string &keyword, int flag) {}
...
void MatcherEngine::ReadKeyword(const std::string &keyword_dir) {
  ...
  // std::vector<std::string> filename = GetFiles(keyword_dir, "txt", false, false);
  // int word_cnt = 0;
  ...
  //   long long id = atoi(filename[i].substr(0, filename[i].length() - 4).c_str());
  //   //        cout << tempfile << endl;
  //   if (tl.size() == 0) {
  //     continue;
  //   }
  ...
  //         0)  //当出现或的关系时，继续添加下一个关键词，即关键词树上不出现或标志
  //     {
  //       // cout << "****appear****" << endl;
  //       continue;
  //     }
  ...
  //     // std::map<std::std::wstring, long long>::iterator iter = Minganci_iddict.find(tl[j]);
  //     if (minganci_num == 0)  //当前map中的key不存在,才在map中添加信息，并同时向AC自动机中插入数据
  //     {
  //       // wcout << tl[j] << endl;
